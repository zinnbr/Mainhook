-- Mainhook, the only one. zinn#0001
...
-- Autowall
local autowalltoggle_key = ui.new_hotkey("RAGE", "Other", "Autowall key")

-- Resolver on/off
local onoff_key = ui.new_hotkey("RAGE", "Other", "Resolver on/off key")

-- Resolver override 
local enemies_only = true
local legit_aa_override_key = ui.new_hotkey("RAGE", "Other", "Legit AA Override")
local last_tick = 0
...
local crosshair_mindmg = ui.new_checkbox("RAGE", "Other", "Crosshair minimum damage")
...
client.set_event_callback("paint", function()
	if not ui.get(rainbow_check) then return end
	for i=1, 64 do
...
client.set_event_callback("paint", function()
	if ui.get(aa_correction) then
		...
		elseif yaw_set == -60 then
			...
			renderer.indicator(60, 255, 70, 240, "R: Skeet")
		end
	end
end)
...
client.set_event_callback("paint", function()
	if ui.get(crosshair_mindmg) then
...
client.set_event_callback("setup_command", function() 
	...
	if ui.get(hide_head) then
		if ui.get(hide_head_key) then
			...
			ui.set(antiaim_fake_yaw_limit, math.random(0, 60))
			return
		else
			ui.set(antiaim_yaw, "Off")
			ui.set(antiaim_pitch, "Off")
	...
	if ui.get(legit_aa) then
		if ui.get(legit_aa_type) == "Anti-resolver" then
			ui.set(antiaim_fake_yaw_limit, math.random(0, 60))
		elseif ui.get(legit_aa_type) == "Legit anti-resolver" then
			ui.set(antiaim_fake_yaw_limit, math.random(37, 60))
		end
